...
# Train Arabic SAEs for target layers: 0, 3, 6, 9, 12, 15, 17
# These match the English SAEs for cross-lingual comparison
...
# Check if layer checkpoints exist for Arabic
CHUNK_COUNT=$(ls checkpoints/full_layers_ncc/layer_checkpoints/layer_${LAYER}_arabic_*.pt 2>/dev/null | wc -l)
echo "Found $CHUNK_COUNT Arabic chunks for layer $LAYER"

if [ $CHUNK_COUNT -lt 50 ]; then
    echo "ERROR: Not enough Arabic chunks for layer $LAYER (need at least 50, have $CHUNK_COUNT)"
    exit 1
fi
