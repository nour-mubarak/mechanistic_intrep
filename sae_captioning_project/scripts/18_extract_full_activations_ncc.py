...
from transformers import AutoModelForCausalLM, AutoModelForVision2Seq, AutoProcessor, AutoModel
...
def load_model(config: dict):
    ...
    # Try different model classes for vision-language models
    model = None
    for model_class in [AutoModelForVision2Seq, AutoModelForCausalLM, AutoModel]:
        try:
            logger.info(f"Trying to load with {model_class.__name__}...")
            ...
            logger.info(f"Successfully loaded with {model_class.__name__}")
            break
        except (ValueError, TypeError) as e:
            logger.warning(f"{model_class.__name__} failed: {e}")
            continue
    
    if model is None:
        raise ValueError(f"Could not load model {model_name} with any auto class")
    
    model.eval()

    logger.info(f"Model loaded with dtype {dtype}")

    # Get model architecture info - PaLiGemma has language_model.model.layers
    if hasattr(model, 'language_model'):
        if hasattr(model.language_model, 'model') and hasattr(model.language_model.model, 'layers'):
            num_layers = len(model.language_model.model.layers)
            logger.info(f"Model has {num_layers} layers (language_model.model.layers)")
            ...
            logger.warning("Could not determine number of layers from language_model")
            num_layers = 18  # Default for PaLiGemma-3b
...
def extract_activations_batch_ncc(
    ...
    # Format prompts for vision-language model
    # PaLiGemma uses simple text prompts, not chat templates
    formatted_prompts = []
    for prompt in prompts:
        # For PaLiGemma, just use the prompt directly
        # The format is simply the text that should follow the image tokens
        formatted_prompts.append(prompt)

    # Prepare inputs - PaLiGemma processor handles image+text directly
    try:
        inputs = processor(
            images=images,
        ...
        logger.warning(f"Standard processing failed: {e}, trying alternative format")
        # Alternative: process each sample separately
        all_inputs = []
        for img, txt in zip(images, formatted_prompts):
            inp = processor(images=img, text=txt, return_tensors="pt")
            all_inputs.append(inp)
        # Stack inputs (simplified - may need adjustment)
        inputs = {k: torch.cat([x[k] for x in all_inputs], dim=0) for k in all_inputs[0].keys()}
    ...
    for layer_idx in layers:
        # Access language model layers - PaLiGemma has language_model.model.layers
        layer = None
        if hasattr(model, 'language_model'):
            if hasattr(model.language_model, 'model') and hasattr(model.language_model.model, 'layers'):
...
def main():
    ...
    for language in args.languages:
        ...
        # NCC: Check for existing chunks and resume from last checkpoint
        existing_chunks = list(layer_checkpoints_dir.glob(f'layer_{layers[0]}_{language}_chunk_*.pt'))
        resume_chunk_idx = len(existing_chunks)
        resume_sample_idx = resume_chunk_idx * checkpoint_interval
        
        if resume_sample_idx > 0:
            logger.info(f"Found {resume_chunk_idx} existing chunks for layer {layers[0]}, resuming from sample {resume_sample_idx}")
        ...
        chunk_genders = []
        chunk_image_ids = []
        chunk_files = list(range(resume_chunk_idx))  # Track existing chunks

        start_time = time.time()
        samples_processed = 0

        for batch_idx in tqdm(range(resume_sample_idx, len(dataset), batch_size), desc=f"Processing {language}"):
            # Get batch
            ...
            try:
                ...
                # Accumulate to chunk (will be saved to disk periodically)
                for layer in layers:
            ...
            # Save checkpoint periodically - directly to disk (NCC memory-efficient)
            if samples_processed >= checkpoint_interval or batch_idx + batch_size >= len(dataset):
                if chunk_activations[layers[0]]:  # Check if we have any data
                    ...
                    # NCC: Save chunks to disk immediately instead of accumulating in memory
                    chunk_idx = len(chunk_files)
                    for layer in layers:
                        if chunk_activations[layer]:
                            layer_act = torch.cat(chunk_activations[layer], dim=0)
                            chunk_path = layer_checkpoints_dir / f'layer_{layer}_{language}_chunk_{chunk_idx:04d}.pt'
                            torch.save({
                    ...
                    logger.info(f"Saved chunk {chunk_idx} to disk")
        ...
        # NCC: Final merge from disk chunks - memory efficient
        logger.info(f"\nMerging {len(chunk_files)} chunks for {len(layers)} layers ({language})...")

        # Merge chunks from disk for each layer
        for layer in tqdm(layers, desc="Merging layers"):
            all_activations = []
            all_genders = []
            all_image_ids = []
            ...
            for chunk_idx in range(len(chunk_files)):
                ...
                if chunk_path.exists():
                    ...
                    all_activations.append(chunk_data['activations'])
                    if chunk_idx == 0:  # Only need metadata from first chunk per layer
                        all_genders = chunk_data['genders']
                        all_image_ids = chunk_data['image_ids']
                    else:
                        all_genders.extend(chunk_data['genders'])
                        all_image_ids.extend(chunk_data['image_ids'])
                    del chunk_data
            ...
            if all_activations:
                ...
                logger.info(f"Layer {layer}: {layer_activations.shape}, {len(all_image_ids)} samples")

                # Save final layer checkpoint
        ...
        logger.info(f"Completed {language} extraction")
